...
describe('shotProcessor', () => {
  ...
  it('derives timing metrics and ellipse', () => {
    const shot = {
      pitch: Array.from({ length: 16 }, (_, index) => index * 0.01),
      yaw: Array.from({ length: 16 }, (_, index) => Math.sin(index / 2) * 0.02),
      shot_index: 4,
      sample_rate: 10,
      trigger_hold: '0.20',
      trigger_pull: '0.08',
    ...
    expect(pre.post_shot_stability_500ms_mm).not.toBeNull();
    if (pre.hold_ellipse) {
  ...
  it('derives elapsed session time from cumulative split', () => {
    const shots = addSessionElapsedTimes([
      { split_s: 0, session_elapsed_s: null } as any,
      { split_s: 12.4, session_elapsed_s: null } as any,
      { split_s: 13.6, session_elapsed_s: null } as any
    ]);
    expect(shots[0].session_elapsed_s).toBe(0);
    expect(shots[1].session_elapsed_s).toBeCloseTo(12.4);
    expect(shots[2].session_elapsed_s).toBeCloseTo(26);
  });

  it('stops elapsed session time when split data becomes invalid', () => {
    const shots = addSessionElapsedTimes([
      { split_s: 0, session_elapsed_s: null } as any,
      { split_s: 12.4, session_elapsed_s: null } as any,
      { split_s: null, session_elapsed_s: null } as any,
      { split_s: 5, session_elapsed_s: null } as any
    ]);
    expect(shots[2].session_elapsed_s).toBeNull();
    expect(shots[3].session_elapsed_s).toBeNull();
  });
