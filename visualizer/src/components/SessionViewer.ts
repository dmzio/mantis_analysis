  ...
  template: `
      <div>
        ...
        <div ref="controls" id="controls" style="display:none">
          ...
          <select ref="corrSelect" id="corr-select" style="margin-left:8px">
            <option value="none">No Correction</option>
            <option value="session">Session Drift</option>
          </select>
        </div>
    ...
    `,
  data() {
    return { session: null as any, shots: [] as any[], current: 0, driftEstimate: null as SessionDriftEstimate | null };
  },
  mounted() {
    ...
    (this.$refs.corrSelect as HTMLSelectElement).addEventListener('change', () => this.drawCanvas());
  },
  methods: {
    toRelativeCoords(shot: ShotData) {
      ...
      const yaw = working.yaw ?? [];
      const hold_index = working.hold_index;
    ...
    loadSession(obj: any) {
      ...
      this.driftEstimate = estimateSessionDrift(this.shots) || null;
