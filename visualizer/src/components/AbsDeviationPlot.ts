...
import { ProcessedShot, absDeviationArray } from '../shotProcessor';

export default defineComponent({
  name: 'AbsDeviationPlot',
  components: { Chart },
  ...
  setup(props) {
    ...
    const build = () => {
      ...
      const arr = (props.shot.abs_deviation_moa ??
        absDeviationArray(props.shot.rel_pitch_moa, props.shot.rel_yaw_moa)).slice(start);
      ...
      chartData.value = {
        labels,
        datasets: [{ label: 'Deviation', data: arr, borderColor: '#37b24d', fill: false, pointRadius: 0 }]
      };
    };
  ...
  template: `
    <div class="abs-deviation-plot">
      <h4>Absolute deviation</h4>
